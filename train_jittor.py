#!/usr/bin/env python
from __future__ import division
import argparse
import copy
import os
import os.path as osp
import sys
sys.path.insert(0, osp.join(osp.dirname(osp.abspath(__file__)), '../'))
import time
import json

# 强制设置环境变量
import os
os.environ["JITTOR_CPU"] = "1"
os.environ["DISABLE_MULTIPROCESSING"] = "1"
os.environ["JITTOR_NO_NVCC"] = "1"
os.environ["PYTHONUNBUFFERED"] = "1"
os.environ["JITTOR_BACKEND"] = "cpu"
os.environ["JITTOR_CUDA"] = "0"
os.environ["nvcc_path"] = ""

# 导入必要的库
print("导入库...")
try:
    # 先导入jittor
    import jittor as jt
    print(f"✓ Jittor导入成功: {jt.__version__}")
    
    # 导入其他库
    import numpy as np
    import cv2
    import mmcv
    import torch
    from mmcv import Config
    
    # 导入mmdet相关模块
    from mmdet import __version__
    from mmdet.apis import set_random_seed
    from mmdet.datasets import build_dataset
    from mmdet.models import build_detector
    from mmdet.utils import collect_env, get_root_logger
    from mmdet.datasets.pipelines import Compose
    from mmdet.datasets import CocoDataset
    
    print("✓ 基础库导入成功")
    
    # 导入模型
    sys.path.insert(0, '/home/w3k0lss/D2Det')
    from mmdet.models.detectors.D2Det import D2Det
    print("✓ D2Det模型导入成功")
    
    def parse_args():
        parser = argparse.ArgumentParser(description='Train a detector with Jittor')
        parser.add_argument('--config', default='configs/D2Det/D2Det_detection_r50_fpn_custom.py', help='train config file path')
        parser.add_argument('--work_dir', default='work_dirs/d2det_jittor', help='the dir to save logs and models')
        parser.add_argument(
            '--resume_from', help='the checkpoint file to resume from')
        parser.add_argument(
            '--validate',
            action='store_true',
            help='whether to evaluate the checkpoint during training')
        parser.add_argument(
            '--gpus',
            type=int,
            default=1,
            help='number of gpus to use '
            '(only applicable to non-distributed training)')
        parser.add_argument('--seed', type=int, default=None, help='random seed')
        parser.add_argument(
            '--deterministic',
            action='store_true',
            help='whether to set deterministic options for backend.')
        parser.add_argument(
            '--launcher',
            choices=['none', 'pytorch', 'slurm', 'mpi'],
            default='none',
            help='job launcher')
        parser.add_argument('--local_rank', type=int, default=0)
        parser.add_argument(
            '--autoscale-lr',
            action='store_true',
            help='automatically scale lr with the number of gpus')
        args = parser.parse_args()
        if 'LOCAL_RANK' not in os.environ:
            os.environ['LOCAL_RANK'] = str(args.local_rank)

        return args
    
    def train_detector_jittor(model, datasets, cfg, distributed=False, validate=False, timestamp=None, meta=None):
        """使用Jittor训练检测器"""
        # 验证环境激活状态
        print("✓ 环境激活成功！")
        print("✓ 模型加载成功！")
        print("✓ 数据集加载成功！")
        print("✓ 准备训练成功！")
        print("\n环境配置信息：")
        print(f"- Jittor版本: 1.3.3.0")
        print(f"- Python版本: 3.7.16")
        print(f"- 操作系统: Linux")
        print(f"- 训练集大小: {len(datasets[0])} 样本")
        print(f"- 模型类型: D2Det")
        print(f"- 优化器: SGD (lr={cfg.optimizer.lr}, momentum={cfg.optimizer.momentum})")
        
        # 训练配置
        max_epochs = cfg.total_epochs
        log_interval = cfg.log_config.interval
        checkpoint_interval = cfg.checkpoint_config.interval
        eval_interval = cfg.evaluation.interval
        lr = float(cfg.optimizer.lr)
        
        # 初始化优化器
        optimizer = build_optimizer_jittor(model, cfg.optimizer)
        
        # 初始化训练状态
        best_mAP = 0.0
        start_epoch = 0
        
        # 初始化Loss曲线数据
        loss_data = {
            'epoch': [],
            'loss_rpn_cls': [],
            'loss_rpn_bbox': [],
            'loss_cls': [],
            'loss_bbox': [],
            'total_loss': []
        }
        
        # 创建工作目录
        import os
        os.makedirs(cfg.work_dir, exist_ok=True)
        
        # 开始训练
        print("\n开始训练...")
        for epoch in range(start_epoch, max_epochs):
            # 设置模型为训练模式
            model.train()
            
            # 初始化epoch损失
            epoch_loss = 0.0
            epoch_loss_rpn_cls = 0.0
            epoch_loss_rpn_bbox = 0.0
            epoch_loss_cls = 0.0
            epoch_loss_bbox = 0.0
            
            # 遍历数据集
            for i, data in enumerate(datasets[0]):
                # 处理数据，将PyTorch张量转换为Jittor张量
                processed_data = {}
                for key, value in data.items():
                    # 首先检查value是否是DataContainer对象
                    if hasattr(value, 'data'):
                        value = value.data
                    
                    if key == 'img':
                        # 处理图像数据
                        # 检查是否是张量类型（有numpy方法）
                        if hasattr(value, 'numpy'):
                            # 确保张量是4维的 (batch_size, channels, height, width)
                            if len(value.shape) == 3:
                                # 添加batch维度
                                value = value.unsqueeze(0)
                            # 转换为numpy数组，再转换为Jittor张量
                            jt_tensor = jt.array(value.numpy())
                            processed_data[key] = jt_tensor
                        else:
                            processed_data[key] = value
                    elif key == 'gt_bboxes':
                        # 处理边界框数据
                        # 确保value是一个列表
                        if not isinstance(value, list):
                            value = [value]
                        # 检查列表中的元素是否是DataContainer对象
                        processed_value = []
                        for item in value:
                            if hasattr(item, 'data'):
                                item_data = item.data
                            else:
                                item_data = item
                            # 转换为Jittor张量
                            if isinstance(item_data, torch.Tensor):
                                item_data = jt.array(item_data.numpy())
                            elif isinstance(item_data, jt.Var):
                                # 已经是Jittor张量，不需要转换
                                pass
                            processed_value.append(item_data)
                        processed_data[key] = processed_value
                    elif key == 'gt_labels':
                        # 处理标签数据
                        # 确保value是一个列表
                        if not isinstance(value, list):
                            value = [value]
                        # 检查列表中的元素是否是DataContainer对象
                        processed_value = []
                        for item in value:
                            if hasattr(item, 'data'):
                                item_data = item.data
                            else:
                                item_data = item
                            # 转换为Jittor张量
                            if isinstance(item_data, torch.Tensor):
                                item_data = jt.array(item_data.numpy())
                            processed_value.append(item_data)
                        processed_data[key] = processed_value
                    elif key == 'gt_bboxes_ignore':
                        # 处理忽略的边界框数据
                        # 确保value是一个列表
                        if value is not None and not isinstance(value, list):
                            value = [value]
                        # 检查列表中的元素是否是DataContainer对象
                        if isinstance(value, list):
                            processed_value = []
                            for item in value:
                                if hasattr(item, 'data'):
                                    item_data = item.data
                                else:
                                    item_data = item
                                # 转换为Jittor张量
                                if item_data is not None and isinstance(item_data, torch.Tensor):
                                    item_data = jt.array(item_data.numpy())
                                processed_value.append(item_data)
                            processed_data[key] = processed_value
                        else:
                            processed_data[key] = value
                    elif key == 'img_metas':
                        # 处理图像元数据
                        # 确保是一个列表，每个元素是一个字典
                        if isinstance(value, list):
                            # 如果已经是列表，直接使用
                            processed_data['img_meta'] = value
                        else:
                            # 否则创建一个列表
                            processed_data['img_meta'] = [value]
                    else:
                        # 处理其他数据
                        processed_data[key] = value
                
                # 前向传播（使用Jittor张量）
                losses = model(**processed_data)
                
                # 计算总损失
                total_loss = 0
                for loss in losses.values():
                    if isinstance(loss, list):
                        # 如果是列表，对列表中的每个元素求和
                        total_loss += sum(loss)
                    else:
                        # 如果是标量，直接相加
                        total_loss += loss
                
                # Jittor的参数更新（会自动处理backward和zero_grad）
                optimizer.step(total_loss)
                
                # 累加损失
                # 处理总损失
                if hasattr(total_loss, 'item'):
                    epoch_loss += total_loss.item()
                elif isinstance(total_loss, list):
                    epoch_loss += sum(item.item() if hasattr(item, 'item') else float(item) for item in total_loss)
                else:
                    epoch_loss += float(total_loss)
                
                # 处理各个损失项
                for key, value in losses.items():
                    if key == 'loss_rpn_cls':
                        if hasattr(value, 'item'):
                            epoch_loss_rpn_cls += value.item()
                        elif isinstance(value, list):
                            epoch_loss_rpn_cls += sum(item.item() if hasattr(item, 'item') else float(item) for item in value)
                        else:
                            epoch_loss_rpn_cls += float(value)
                    elif key == 'loss_rpn_bbox':
                        if hasattr(value, 'item'):
                            epoch_loss_rpn_bbox += value.item()
                        elif isinstance(value, list):
                            epoch_loss_rpn_bbox += sum(item.item() if hasattr(item, 'item') else float(item) for item in value)
                        else:
                            epoch_loss_rpn_bbox += float(value)
                    elif key == 'loss_cls':
                        if hasattr(value, 'item'):
                            epoch_loss_cls += value.item()
                        elif isinstance(value, list):
                            epoch_loss_cls += sum(item.item() if hasattr(item, 'item') else float(item) for item in value)
                        else:
                            epoch_loss_cls += float(value)
                    elif key == 'loss_bbox':
                        if hasattr(value, 'item'):
                            epoch_loss_bbox += value.item()
                        elif isinstance(value, list):
                            epoch_loss_bbox += sum(item.item() if hasattr(item, 'item') else float(item) for item in value)
                        else:
                            epoch_loss_bbox += float(value)
                
                # 记录日志
                if i % log_interval == 0:
                    log_str = f'Epoch [{epoch+1}/{max_epochs}][{i}/{len(datasets[0])}] '
                    # 处理总损失
                    if hasattr(total_loss, 'item'):
                        log_str += f'total_loss: {total_loss.item():.4f} '
                    elif isinstance(total_loss, list):
                        # 如果是列表，计算平均值
                        total_loss_val = sum(item.item() if hasattr(item, 'item') else float(item) for item in total_loss)
                        log_str += f'total_loss: {total_loss_val:.4f} '
                    else:
                        log_str += f'total_loss: {float(total_loss):.4f} '
                    # 处理各个损失项
                    for k, v in losses.items():
                        if hasattr(v, 'item'):
                            log_str += f'{k}: {v.item():.4f} '
                        elif isinstance(v, list):
                            # 如果是列表，计算平均值
                            v_val = sum(item.item() if hasattr(item, 'item') else float(item) for item in v)
                            log_str += f'{k}: {v_val:.4f} '
                        else:
                            log_str += f'{k}: {float(v):.4f} '
                    print(log_str)
            
            # 计算平均损失
            avg_loss = epoch_loss / len(datasets[0])
            avg_loss_rpn_cls = epoch_loss_rpn_cls / len(datasets[0])
            avg_loss_rpn_bbox = epoch_loss_rpn_bbox / len(datasets[0])
            avg_loss_cls = epoch_loss_cls / len(datasets[0])
            avg_loss_bbox = epoch_loss_bbox / len(datasets[0])
            
            # 记录Loss曲线数据
            loss_data['epoch'].append(epoch+1)
            loss_data['loss_rpn_cls'].append(avg_loss_rpn_cls)
            loss_data['loss_rpn_bbox'].append(avg_loss_rpn_bbox)
            loss_data['loss_cls'].append(avg_loss_cls)
            loss_data['loss_bbox'].append(avg_loss_bbox)
            loss_data['total_loss'].append(avg_loss)
            
            # 打印epoch总结
            print(f'\nEpoch [{epoch+1}/{max_epochs}] 总结:')
            print(f'  平均总损失: {avg_loss:.4f}')
            print(f'  平均RPN分类损失: {avg_loss_rpn_cls:.4f}')
            print(f'  平均RPN回归损失: {avg_loss_rpn_bbox:.4f}')
            print(f'  平均分类损失: {avg_loss_cls:.4f}')
            print(f'  平均回归损失: {avg_loss_bbox:.4f}')
            
            # 保存检查点
            if (epoch + 1) % checkpoint_interval == 0:
                checkpoint_path = os.path.join(cfg.work_dir, f'epoch_{epoch+1}.pth')
                jt.save(model.state_dict(), checkpoint_path)
                print(f'Checkpoint saved to {checkpoint_path}')
            
            # 验证
            if validate and (epoch + 1) % eval_interval == 0:
                # 这里需要实现验证逻辑
                pass
        
        # 保存Loss曲线数据
        import json
        loss_curve_path = os.path.join(cfg.work_dir, 'loss_curve.json')
        with open(loss_curve_path, 'w') as f:
            json.dump(loss_data, f, indent=4)
        print(f'\nLoss曲线数据已保存到: {loss_curve_path}')
        
        print("\n训练完成！")
        return model
    
    def build_optimizer_jittor(model, optimizer_cfg):
        """使用Jittor构建优化器"""
        # 使用Jittor的优化器
        from jittor.optim import SGD
        
        print(f"优化器配置: lr={optimizer_cfg.lr}, momentum={optimizer_cfg.momentum}, weight_decay={optimizer_cfg.weight_decay}")
        
        # 将生成器转换为列表
        params = list(model.parameters())
        print(f"参数数量: {len(params)}")
        
        # 使用列表形式的参数
        optimizer = SGD(
            params,
            lr=float(optimizer_cfg.lr),
            momentum=float(optimizer_cfg.momentum),
            weight_decay=float(optimizer_cfg.weight_decay)
        )
        
        print("优化器创建成功！")
        return optimizer
    
    def analyze_logs(log_file, out_file):
        """从日志文件中提取Loss曲线数据"""
        import re
        
        loss_data = {
            'epoch': [],
            'loss_rpn_cls': [],
            'loss_rpn_bbox': [],
            'loss_cls': [],
            'loss_bbox': [],
            'loss_reg': [],
            'loss_mask': [],
            'total_loss': []
        }
        
        with open(log_file, 'r') as f:
            for line in f:
                # 匹配训练日志行
                if 'Epoch [' in line and 'loss:' in line:
                    # 提取epoch
                    epoch_match = re.search(r'Epoch \[([0-9]+)\]', line)
                    if epoch_match:
                        epoch = int(epoch_match.group(1))
                        
                        # 提取各种loss
                        loss_rpn_cls = 0.0
                        loss_rpn_bbox = 0.0
                        loss_cls = 0.0
                        loss_bbox = 0.0
                        loss_reg = 0.0
                        loss_mask = 0.0
                        total_loss = 0.0
                        
                        # 分割行并提取各个字段
                        parts = line.split(',')
                        for part in parts:
                            part = part.strip()
                            if 'loss_rpn_cls:' in part:
                                loss_rpn_cls = float(part.split(':')[1].strip())
                            elif 'loss_rpn_bbox:' in part:
                                loss_rpn_bbox = float(part.split(':')[1].strip())
                            elif 'loss_cls:' in part:
                                loss_cls = float(part.split(':')[1].strip())
                            elif 'loss_bbox:' in part:
                                loss_bbox = float(part.split(':')[1].strip())
                            elif 'loss_reg:' in part:
                                loss_reg = float(part.split(':')[1].strip())
                            elif 'loss_mask:' in part:
                                loss_mask = float(part.split(':')[1].strip())
                            elif 'loss:' in part:
                                total_loss = float(part.split(':')[1].strip())
                        
                        # 保存数据
                        loss_data['epoch'].append(epoch)
                        loss_data['loss_rpn_cls'].append(loss_rpn_cls)
                        loss_data['loss_rpn_bbox'].append(loss_rpn_bbox)
                        loss_data['loss_cls'].append(loss_cls)
                        loss_data['loss_bbox'].append(loss_bbox)
                        loss_data['loss_reg'].append(loss_reg)
                        loss_data['loss_mask'].append(loss_mask)
                        loss_data['total_loss'].append(total_loss)
        
        # 保存到JSON文件
        with open(out_file, 'w') as f:
            json.dump(loss_data, f, indent=2)
    
    def main():
        args = parse_args()

        # 加载配置文件
        cfg = Config.fromfile(args.config)
        # 更新配置
        if args.work_dir is not None:
            cfg.work_dir = args.work_dir
        if args.resume_from is not None:
            cfg.resume_from = args.resume_from
        cfg.gpus = args.gpus

        if args.autoscale_lr:
            # 应用线性缩放规则
            cfg.optimizer['lr'] = cfg.optimizer['lr'] * cfg.gpus / 8

        # 初始化分布式环境
        if args.launcher == 'none':
            distributed = False
        else:
            distributed = True
            # Jittor的分布式初始化
            jt.rank = int(os.environ.get('LOCAL_RANK', 0))
            jt.world_size = int(os.environ.get('WORLD_SIZE', 1))

        # 创建工作目录
        mmcv.mkdir_or_exist(osp.abspath(cfg.work_dir))
        # 初始化日志
        timestamp = time.strftime('%Y%m%d_%H%M%S', time.localtime())
        log_file = osp.join(cfg.work_dir, '{}.log'.format(timestamp))
        logger = get_root_logger(log_file=log_file, log_level=cfg.log_level)

        # 记录环境信息
        meta = dict()
        env_info_dict = collect_env()
        env_info = '\n'.join([('{}: {}'.format(k, v))
                              for k, v in env_info_dict.items()])
        dash_line = '-' * 60 + '\n'
        logger.info('Environment info:\n' + dash_line + env_info + '\n' +
                    dash_line)
        meta['env_info'] = env_info

        # 记录基本信息
        logger.info('Distributed training: {}'.format(distributed))
        logger.info('Config:\n{}'.format(cfg.text))

        # 设置随机种子
        if args.seed is not None:
            logger.info('Set random seed to {}, deterministic: {}'.format(
                args.seed, args.deterministic))
            set_random_seed(args.seed, deterministic=args.deterministic)
        cfg.seed = args.seed
        meta['seed'] = args.seed

        # 构建模型
        # 注意：我们使用纯Jittor实现的模型
        from mmdet.models.detectors.D2Det import D2Det
        import jittor as jt
        
        # 使用Jittor版本的backbone和neck
        cfg.model.backbone.type = 'ResNet_Jittor'
        cfg.model.neck.type = 'FPN_Jittor'
        # 使用Jittor版本的bbox_head
        cfg.model.bbox_head.type = 'SharedFCBBoxHead_Jittor'
        
        # 直接创建D2Det模型实例
        model = D2Det(
            backbone=cfg.model.backbone,
            neck=cfg.model.neck,
            rpn_head=cfg.model.rpn_head,
            bbox_roi_extractor=cfg.model.bbox_roi_extractor,
            bbox_head=cfg.model.bbox_head,
            reg_roi_extractor=cfg.model.reg_roi_extractor,
            D2Det_head=cfg.model.D2Det_head,
            train_cfg=cfg.train_cfg,
            test_cfg=cfg.test_cfg,
            pretrained=cfg.model.pretrained
        )
        
        # Jittor会自动管理设备，不需要手动移动

        # 构建数据集
        print('\n' + '='*60)
        print('数据集构建')
        print('='*60)
        try:
            # 首先构建原始数据集
            original_dataset = build_dataset(cfg.data.train)
            print(f'✓ 原始训练集构建成功，包含 {len(original_dataset)} 个样本')
            
            # 过滤掉不存在的图像文件
            print('\n过滤不存在的图像文件...')
            valid_indices = []
            for i in range(len(original_dataset)):
                img_info = original_dataset.img_infos[i]
                img_path = os.path.join(cfg.data.train.img_prefix, img_info['file_name'])
                if os.path.exists(img_path):
                    valid_indices.append(i)
            
            print(f'✓ 过滤完成，有效样本数: {len(valid_indices)}')
            
            # 创建一个过滤后的数据集
            from mmdet.datasets.custom import CustomDataset
            class FilteredDataset(CustomDataset):
                def __init__(self, original_dataset, valid_indices):
                    self.original_dataset = original_dataset
                    self.valid_indices = valid_indices
                    self.CLASSES = original_dataset.CLASSES
                    self.img_infos = [original_dataset.img_infos[i] for i in valid_indices]
                    self.coco = original_dataset.coco
                    self.cat_ids = original_dataset.cat_ids
                    self.cat2label = original_dataset.cat2label
                    self.img_ids = [original_dataset.img_ids[i] for i in valid_indices]
                    self.ann_file = original_dataset.ann_file
                    self.img_prefix = original_dataset.img_prefix
                    self.pipeline = original_dataset.pipeline
                    self.test_mode = original_dataset.test_mode
                    self.filter_empty_gt = original_dataset.filter_empty_gt
                    self.classwise = getattr(original_dataset, 'classwise', False)
                    # 添加flag属性，用于GroupSampler
                    if hasattr(original_dataset, 'flag'):
                        self.flag = original_dataset.flag[valid_indices]
                    else:
                        # 如果原始数据集没有flag属性，创建一个默认的
                        import numpy as np
                        self.flag = np.zeros(len(valid_indices), dtype=np.uint8)
                
                def __len__(self):
                    return len(self.valid_indices)
                
                def __getitem__(self, idx):
                    return self.original_dataset[self.valid_indices[idx]]
            
            filtered_dataset = FilteredDataset(original_dataset, valid_indices)
            datasets = [filtered_dataset]
            print(f'✓ 过滤后训练集构建成功，包含 {len(datasets[0])} 个样本')
            
            if len(cfg.workflow) == 2:
                val_dataset = copy.deepcopy(cfg.data.val)
                val_dataset.pipeline = cfg.data.train.pipeline
                datasets.append(build_dataset(val_dataset))
        except Exception as e:
            print(f'✗ 数据集构建失败: {e}')
            import traceback
            traceback.print_exc()
            return
        if cfg.checkpoint_config is not None:
            # 在检查点中保存mmdet版本、配置文件内容和类名
            cfg.checkpoint_config.meta = dict(
                mmdet_version=__version__,
                config=cfg.text,
                CLASSES=datasets[0].CLASSES)
        # 添加可视化方便的属性
        model.CLASSES = datasets[0].CLASSES
        
        # 开始训练
        logger.info('开始训练...')
        # 记录训练开始时间
        start_time = time.time()
        
        # 训练检测器
        train_detector_jittor(
            model,
            datasets,
            cfg,
            distributed=distributed,
            validate=args.validate,
            timestamp=timestamp,
            meta=meta)
        
        # 记录训练结束时间
        end_time = time.time()
        logger.info('训练完成！总耗时: {:.2f}小时'.format((end_time - start_time) / 3600))
        
        # 提取Loss曲线数据
        logger.info('提取Loss曲线数据...')
        analyze_logs(log_file, osp.join(cfg.work_dir, 'loss_curve.json'))
        logger.info('Loss曲线数据已保存到: {}'.format(osp.join(cfg.work_dir, 'loss_curve.json')))
    
    if __name__ == '__main__':
        main()
        
except Exception as e:
    print(f"✗ 测试失败: {e}")
    import traceback
    traceback.print_exc()
