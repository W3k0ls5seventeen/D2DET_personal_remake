...
class D2DetHead(nn.Module):
    ...
    def init_weights(self):
        ...
        nn.init.normal_(self.D2Det_reg.weight, std=0.001)
        nn.init.normal_(self.D2Det_mask.weight, std=0.001)
        ...
        if self.MASK_ON:
            ...
            nn.init.normal_(self.fc_instance_iou.weight, std=0.01)
    ...
    def get_target(self, sampling_results):
        ...
        map_size = 7
        targets = torch.zeros((num_rois, 4, map_size, map_size), dtype=torch.float32)
        points = torch.zeros((num_rois, 4, map_size, map_size), dtype=torch.float32)
        masks = torch.zeros((num_rois, 1, map_size, map_size), dtype=torch.float32)
    ...
    def get_target_mask(self, sampling_results, gt_masks, rcnn_train_cfg):
        ...
        instances = mask_target([res.pos_bboxes for res in sampling_results], pos_assigned_gt_inds, gt_masks,
                            rcnn_train_cfg)
        masks = F.interpolate(instances.unsqueeze(0), scale_factor=1 / 4, mode='bilinear').squeeze(0)
    ...
    def get_bboxes_avg(self, det_bboxes, D2Det_pred, D2Det_pred_mask, img_meta):
        ...
        map_size = 7
        targets = torch.zeros((det_bboxes.shape[0], 4, map_size, map_size), dtype=torch.float32, device=D2Det_pred.device)

        idx = (torch.arange(0, map_size).float() + 0.5) / map_size
    ...
    def _get_area_ratio(self, pos_proposals, pos_assigned_gt_inds, gt_masks):
        ...
        if num_pos > 0:
            ...
            area_ratios = torch.tensor(np.stack(area_ratios)).float().to(
                pos_proposals.device)
        else:
            area_ratios = torch.zeros((0, ), device=pos_proposals.device)
        return area_ratios