...
class D2Det(TwoStageDetector):
    def __init__(self,
        ...
        self.MASK_ON = D2Det_head.get('MASK_ON', False)
        if self.MASK_ON:
    ...
    def _random_jitter(self, sampling_results, img_metas, amplitude=0.15):
        ...
        for sampling_result, img_meta in zip(sampling_results, img_metas):
            ...
            if bboxes.numel() == 0:
                continue
            # 确保随机偏移在与bboxes相同的设备上
            device = bboxes.device
            random_offsets = torch.rand(bboxes.shape[0], 4, device=device) * (2 * amplitude) - amplitude
            # before jittering
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            if self.MASK_ON == False:
                ...
                reg_targets = reg_targets.to(device)[sample_idx]
                reg_points = reg_points.to(device)[sample_idx]
                reg_masks = reg_masks.to(device)[sample_idx]
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels,
        ...
        if isinstance(mask_pred, torch.Tensor):
            mask_pred = mask_pred.sigmoid().numpy()
