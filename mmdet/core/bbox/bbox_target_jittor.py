...
def bbox_target_single(pos_bboxes,
    ...
    labels = jt.zeros((num_samples,), dtype=jt.int64)
...
def expand_target(bbox_targets, bbox_weights, labels, num_classes):
    bbox_targets_expand = jt.zeros(
        (bbox_targets.shape[0], 4 * num_classes))
    bbox_weights_expand = jt.zeros(
        (bbox_weights.shape[0], 4 * num_classes))
    for i in jt.nonzero(labels > 0).squeeze(-1):
