...
def delta2bbox(rois,
    ...
    """
    import jittor as jt
    means = jt.array(means).repeat(1, deltas.shape[1] // 4)
    stds = jt.array(stds).repeat(1, deltas.shape[1] // 4)
    ...
    px = ((rois[:, 0] + rois[:, 2]) * 0.5).unsqueeze(1).repeat(1, dx.shape[1])
    py = ((rois[:, 1] + rois[:, 3]) * 0.5).unsqueeze(1).repeat(1, dy.shape[1])
    # Compute width/height of each roi
    pw = (rois[:, 2] - rois[:, 0] + 1.0).unsqueeze(1).repeat(1, dw.shape[1])
    ph = (rois[:, 3] - rois[:, 1] + 1.0).unsqueeze(1).repeat(1, dh.shape[1])
    ...
    if max_shape is not None:
        x1 = x1.clamp(0, max_shape[1] - 1)
        y1 = y1.clamp(0, max_shape[0] - 1)
        x2 = x2.clamp(0, max_shape[1] - 1)
        y2 = y2.clamp(0, max_shape[0] - 1)
    bboxes = jt.stack([x1, y1, x2, y2], dim=-1).reshape(deltas.shape)
    return bboxes
...
def bbox2roi(bbox_list):
    ...
    for img_id, bboxes in enumerate(bbox_list):
        if bboxes.shape[0] > 0:
            ...
            rois = jt.concat([img_inds, bboxes[:, :4]], dim=-1)
        else:
            rois = jt.zeros((0, 5), dtype=bboxes.dtype)
...
def roi2bbox(rois):
    bbox_list = []
    img_ids = jt.unique(rois[:, 0].cpu(), sorted=True)
...
def distance2bbox(points, distance, max_shape=None):
    ...
    Args:
        points (Var): Shape (n, 2), [x, y].
        distance (Var): Distance from the given point to 4
